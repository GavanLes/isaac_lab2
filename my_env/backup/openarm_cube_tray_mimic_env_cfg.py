...
# Custom recorder: save IK target joint positions (action[t]) during data generation
class PostStepJointPosTargetRecorder(RecorderTerm):
    """Recorder term that records the joint position targets (= IK result) after each step."""

    def record_post_step(self):
        joint_pos_target = self._env.scene["robot"].data.joint_pos_target.clone()
        return "joint_pos_target", joint_pos_target


@configclass
class PostStepJointPosTargetRecorderCfg(RecorderTermCfg):
    class_type: type[RecorderTerm] = PostStepJointPosTargetRecorder


@configclass
class OpenArmXRecorderManagerCfg(ActionStateRecorderManagerCfg):
    """Extends default recorders with joint_pos_target (IK target)."""
    record_post_step_joint_pos_target = PostStepJointPosTargetRecorderCfg()
...
def _reset_default_joint_pose(env, env_ids, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
    ...
    if asset.root_physx_view is None:
        return  # scene not written yet — wait for first sim step
    n = len(env_ids)
...
def _randomize_cube_pose(env, env_ids, asset_cfg: SceneEntityCfg = SceneEntityCfg("cube")):
    ...
    for idx, i in enumerate(env_ids.tolist()):
        ...
        x = rx + 0.35 + random.uniform(-0.10, 0.10)
        y = ry + 0.19 + random.uniform(-0.10, 0.10)
        z = _CUBE_START_Z
...
# ---------------------------------------------------------------------------
# Custom cube spawner — EXACT replica of recording's create_dynamic_cube()
# ---------------------------------------------------------------------------


@clone
def _spawn_recording_style_cube(
    prim_path: str,
    cfg: "RecordingCubeCfg",
    **kwargs,
):
    """Spawn a dynamic cuboid prim matching recording's create_dynamic_cube() EXACTLY.

    Flat prim hierarchy: UsdGeomCube + all physics APIs on the SAME prim.
    No extra collision_props/rigid_props/mass_props beyond what the recording sets.
    """
    from pxr import Gf, PhysxSchema, Sdf, UsdGeom, UsdPhysics, UsdShade
    from isaaclab.sim.utils.prims import get_current_stage

    stage = get_current_stage()

    # Remove existing prim if re-spawning (matches recording behavior)
    if stage.GetPrimAtPath(prim_path).IsValid():
        stage.RemovePrim(prim_path)

    # --- Geometry (UsdGeomCube directly on prim_path — flat hierarchy) ---
    ...
    xform.AddTranslateOp().Set(Gf.Vec3d(0, 0, 0))  # pose set by init_state
    ...
    material_path = f"{prim_path}/GreenMaterial"
    material = UsdShade.Material.Define(stage, material_path)
    shader = UsdShade.Shader.Define(stage, f"{material_path}/PreviewSurface")
    shader.CreateIdAttr("UsdPreviewSurface")
    shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(
        Gf.Vec3f(0.0, 0.8, 0.0),
    )
    ...
    physics_material.CreateStaticFrictionAttr(float(cfg.static_friction))
    physics_material.CreateDynamicFrictionAttr(float(cfg.dynamic_friction))
    physics_material.CreateRestitutionAttr(float(cfg.restitution))
...
# ---------------------------------------------------------------------------
# Custom hex prism spawner — EXACT replica of recording's create_dynamic_hexagonal_prism()
# ---------------------------------------------------------------------------


@clone
def _spawn_recording_style_hex_prism(
    prim_path: str,
    cfg: "RecordingHexPrismCfg",
    **kwargs,
):
    """Spawn a dynamic hexagonal prism matching create_dynamic_hexagonal_prism() EXACTLY."""
    import math
    from pxr import Gf, PhysxSchema, Sdf, UsdGeom, UsdPhysics, UsdShade
    from isaaclab.sim.utils.prims import get_current_stage
    ...
    R = float(cfg.side_length)
    hz = float(cfg.height) * 0.5
    ...
    top_c = Gf.Vec3f(0, 0, hz)
    bot_c = Gf.Vec3f(0, 0, -hz)
    points = top_pts + bot_pts + [top_c, bot_c]

    fvc = []
    fvi = []
    for i in range(6):
        fvc.append(3); fvi.extend([12, i, (i + 1) % 6])
    for i in range(6):
        fvc.append(3); fvi.extend([13, 6 + (i + 1) % 6, 6 + i])
    for i in range(6):
        j = (i + 1) % 6
        fvc.append(4); fvi.extend([i, j, 6 + j, 6 + i])

    mesh = UsdGeom.Mesh.Define(stage, prim_path)
    mesh.CreatePointsAttr(points)
    mesh.CreateFaceVertexCountsAttr(fvc)
    mesh.CreateFaceVertexIndicesAttr(fvi)
    mesh.CreateDisplayColorAttr([Gf.Vec3f(cfg.color[0], cfg.color[1], cfg.color[2])])
    ...
    # Material
    material_path = f"{prim_path}/HexMaterial"
    material = UsdShade.Material.Define(stage, material_path)
    shader = UsdShade.Shader.Define(stage, f"{material_path}/PreviewSurface")
    shader.CreateIdAttr("UsdPreviewSurface")
    shader.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(
        Gf.Vec3f(cfg.color[0], cfg.color[1], cfg.color[2]),
    )
    shader.CreateInput("roughness", Sdf.ValueTypeNames.Float).Set(0.45)
    shader.CreateInput("metallic", Sdf.ValueTypeNames.Float).Set(0.0)
    material.CreateSurfaceOutput().ConnectToSource(shader.ConnectableAPI(), "surface")
...
@configclass
class RecordingHexPrismCfg(SpawnerCfg):
    """Config matching recording's create_dynamic_hexagonal_prism() parameters exactly."""

    func: Callable = _spawn_recording_style_hex_prism
    side_length: float = 0.025
    height: float = 0.045
    mass: float = 0.01
    color: tuple = (0.0, 0.35, 0.0)
    static_friction: float = 80.0
    dynamic_friction: float = 80.0
    restitution: float = 0.0


@configclass
class RecordingCubeCfg(SpawnerCfg):
    """Config matching recording's create_dynamic_cube() parameters exactly."""

    func: Callable = _spawn_recording_style_cube
    size: float = 0.04
    height: float = 0.045
    mass: float = 0.05
    static_friction: float = 20.0
    dynamic_friction: float = 20.0
    restitution: float = 0.0
...
class OpenArmCubeTrayEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        # Match record_demo.py: important for gripper/object contacts.
        ...
        # Cube — EXACT replica of recording's create_dynamic_cube()
        # Flat prim hierarchy, same material/friction/mass, no extra collision props
        self.scene.cube = RigidObjectCfg(
            prim_path="{ENV_REGEX_NS}/Cube",
            init_state=RigidObjectCfg.InitialStateCfg(
                pos=(_ROBOT_POS[0] + 0.35, _ROBOT_POS[1] + 0.19, _CUBE_START_Z), rot=[1, 0, 0, 0],
            ),
            spawn=RecordingCubeCfg(),
        )
...
class OpenArmCubeTrayMimicEnvCfg(OpenArmCubeTrayEnvCfg, MimicEnvCfg):
    recorders: OpenArmXRecorderManagerCfg = OpenArmXRecorderManagerCfg()
