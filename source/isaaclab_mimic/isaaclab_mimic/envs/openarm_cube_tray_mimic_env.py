...
class OpenArmCubeTrayMimicEnv(ManagerBasedRLMimicEnv):
    ...
    def __init__(self, cfg, render_mode: str | None = None):
        ...
        self._apply_cube_friction()

    def _apply_cube_friction(self):
        """Verify/override cube mass and friction to match recording.

        cube_physics.usd already has correct values baked in, but we double-check
        here in case PhysX reset or cloning loses any material properties.
        """
        ...
        for env_path in self.scene.env_prim_paths:
            ...
            # Patch all bound materials with high friction + combine mode max
